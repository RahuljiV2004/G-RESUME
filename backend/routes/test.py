...
import noisereduce as nr
from utils.audio_extract1 import extract_audio
from utils.rs import analyze_video
...
@router.post("/upload/")
async def process_video(file: UploadFile = File(...), question: str = Form(...)):
    """Processes the video file, extracts audio & video insights, and analyzes performance."""
    try:
        file_uuid = uuid.uuid4()
        original_filename = file.filename
        extension = original_filename.split(".")[-1] if "." in original_filename else "webm"
        ...
        result = {
            ...
            "posture_analysis": "0.82",
            "eye_contact_score": "0.67",
        }
        ...
        if model:
            transcription = transcribe_audio(denoised_audio)
            result["transcription"] = transcription
            result["snr"] = round(compute_snr(denoised_audio), 2)
            word_count, wpm = analyze_speech_rate(transcription, denoised_audio)
            result["word_count"] = word_count
            result["speech_rate_wpm"] = round(wpm, 2)
            result["filler_words_count"] = detect_filler_words(transcription)

            # AI-based feedback using Cohere
            if co:
                result["answer_feedback"] = check_answer_feedback(question, transcription)

        # **Call video analysis function to get emotion metrics**
        try:
            video_analysis = analyze_video(video_path)  # ✅ Call function
            result["confidence_level"] = video_analysis.get("confidence_level", result["confidence_level"])
            result["posture_analysis"] = video_analysis.get("posture", result["posture_analysis"])
            result["eye_contact_score"] = video_analysis.get("eye_contact", result["eye_contact_score"])
        except Exception as e:
...
def compute_snr(audio_file):
    y, sr = librosa.load(audio_file, sr=None)
    speech_rms = np.mean(librosa.feature.rms(y=y))
    noise_rms = np.mean(librosa.feature.rms(y=y[:sr//2]))
    snr = 20 * np.log10(speech_rms / (noise_rms + 1e-9))
    return snr
